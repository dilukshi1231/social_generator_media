...
def auto_approve_content(content_id: int):
    ...
    try:
        ...
        if content.status == ContentStatus.PENDING_APPROVAL:
            ...
            return {"success": True, "content_id": content_id, "status": "approved"}
...
def regenerate_failed_content():
    ...
    try:
        ...
        failed_contents = (
            db.query(Content)
            .filter(
                Content.status == ContentStatus.FAILED, Content.created_at >= cutoff
            )
            .all()
        )
...
def archive_old_published_content():
    ...
    try:
        ...
        for content in contents:
            ...
            current_data = content.extra_data or {}
            ...
            content.extra_data = updated
            archived += 1

        db.commit()

        return {"archived": archived, "cutoff_date": cutoff_date.isoformat()}
    except Exception as e:
        db.rollback()
        return {"error": str(e)}
    finally:
